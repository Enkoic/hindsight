...
`hindsight` aggregates per-machine activity across **ActivityWatch / Claude Code / Codex CLI / Cursor / VS Code Copilot Chat / ChatGPT export** into a local SQLite store, then asks an LLM to write a daily digest. Output goes to Markdown / JSON / Notion / Obsidian / Slack-or-Discord webhook. There is also a weekly/monthly rollup plus a platform-aware schedule helper for macOS launchd and Linux systemd user timers. See `docs/architecture.md`.

## Where things live

```
hindsight/
├── cli.py             # typer entry points; collect/report/summarize/rollup/export/run/stats/schedule
├── config.py          # env vars → Config; data dir resolution; auto-detected source paths
├── models.py          # Event + fingerprint
├── store.py           # SQLite schema + cache helpers for events / summaries / rollups
├── redact.py          # redact secrets before LLM calls
├── schedule.py        # macOS launchd + Linux systemd --user install/show/uninstall
├── collectors/{base,activitywatch,claude_code,codex,cursor,chatgpt,history,vscode}.py
...
3. Wire into `cli.py::_collectors()` and `KNOWN_SOURCES` (collector) or `cli.py::export()` (exporter).
...
- Don't auto-install the scheduler unless the user asked — `schedule install` is opt-in for a reason on both macOS and Linux.
